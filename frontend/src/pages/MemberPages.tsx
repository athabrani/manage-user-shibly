...
import { FileSpreadsheet, MapPin, Search, ChevronLeft, ChevronRight, Building2, Phone, Map } from 'lucide-react';
...
export default function Members() {
  ...
  useEffect(() => {
    fetchMembers(page);
  }, [page]); // Fetch ulang jika page berubah
    ...
    api.get('/dashboard/recap')
      .then(res => {
        ...
        if (res.data.levelName) {
            setLevelName(res.data.levelName);
        }
      })
      .catch(console.error)
      .finally(() => setLoadingRecap(false));
  };
  const fetchMembers = (pageNum: number) => {
    setLoadingMembers(true);
    api.get(`/members?page=${pageNum}`)
      .then(res => {
...
const downloadExcel = async (endpoint: string, filename: string) => {
    try {
      ...
      alert("Gagal mengunduh file. Pastikan backend berjalan.");
    } finally {
    ...
    <div className="space-y-8 pb-10">
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">Rekapitulasi {levelName}</h2>
            <p className="text-gray-500 text-sm">Data sebaran anggota berdasarkan tingkatan {levelName.toLowerCase()}.</p>
          </div>
          <button 
            onClick={() => downloadExcel('/dashboard/recap', `Rekapitulasi_${levelName}.xlsx`)}
            disabled={downloading}
            className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 shadow-lg shadow-green-100 disabled:opacity-50 disabled:cursor-wait"
          >
            {downloading ? 'Mengunduh...' : <><FileSpreadsheet size={16} /> Export Rekap</>}
          </button>
        </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {loadingRecap ? (
                ...
                <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4 font-mono text-sm text-gray-600 bg-gray-50/30 w-32 border-r border-gray-50">
                    {r.code}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-blue-50 text-blue-600 rounded-lg shrink-0">
                        {levelName === 'Provinsi' ? <Map size={18} /> : 
                         levelName.includes('Kabupaten') ? <Building2 size={18} /> : 
                         <MapPin size={18} />}
                      </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="text-xs text-gray-500 font-medium">
                       {r.hierarchy === 'Tingkat Provinsi' ? (
                         <span className="italic text-gray-400">Tingkat Provinsi</span>
                       ) : (
                         <div className="flex flex-col gap-0.5">
                            <span className="uppercase text-[10px] text-gray-400 tracking-wider">Induk Wilayah:</span>
                            <span className="text-gray-700">{r.hierarchy}</span>
                         </div>
                       )}
                    </div>
                  </td>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-50">
                {loadingMembers ? (
                   ...
                   <tr><td colSpan={4} className="p-8 text-center text-gray-500">Tidak ada data anggota.</td></tr>
                ) : members.map((m) => (
                  <tr key={m.ktp} className="hover:bg-gray-50/50 transition-colors align-top">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 font-mono mt-0.5">KTP: {m.ktp}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="text-xs space-y-1.5">
                        ...
                        <div className="flex items-center gap-2 pl-1 border-l-2 border-gray-200">
                           <span className="text-gray-500">Kec. {m.districtName}</span>
                           <span className="text-gray-300">•</span>
                           <span className="text-gray-500">Kel. {m.villageName}</span>
                        </div>
                      </div>
                    </td>
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-gray-50">
             ...
             <div className="flex gap-2">
               <button 
                 ...
                 disabled={page === 1}
                 className="p-2 border rounded-lg bg-white hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
               >
                 <ChevronLeft size={16} />
               </button>
               <button 
